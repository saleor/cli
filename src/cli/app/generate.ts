...
export const handler = async (argv: Arguments<Options>) => {
  ...
  switch (resource) {
    case 'webhook':
      ...
      const appWebhookPath = path.join('pages', 'api', 'webhooks', `${webhookName}.ts`);
      const webhookPath = path.join(process.cwd(), appWebhookPath);
      const rootPath = path.resolve(__dirname, '..', '..', '..');
      const webhookHandlerFn = await fs.readFile(path.join(rootPath, 'template', 'webhook-handler.ts'), 'utf-8');
      await fs.outputFile(webhookPath, webhookHandlerFn);

      console.log(`\nGenerated a webhook handler for the '${form}' event in '${appWebhookPath}'`)

      const subscriptionPath = path.join(process.cwd(), 'graphql', 'subscriptions', `${camelcaseName}Subscription.graphql`);
      const subscriptionTemplatePath = path.join(rootPath, 'template', 'event-subscription.graphql');
      const subscriptionTemplate = await fs.readFile(subscriptionTemplatePath, 'utf-8');

      const operationName = uncapitalize(webhookName.split('-').slice(0, -1).map(capitalize).join(''))
      const subscriptionCompiledTemplate = render(subscriptionTemplate, { name: camelcaseName, operationName })
